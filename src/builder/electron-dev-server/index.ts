...
import { BrowserBuilder, NormalizedBrowserBuilderSchema, getBrowserLoggingCb } from '@angular-devkit/build-angular/src/browser/';
...
export class DevServerBuilder implements Builder<DevServerBuilderOptions> {
    ...
    constructor(
        ...
        this.host = host ? host : new virtualFs.AliasHost(this.context.host as virtualFs.Host<fs.Stats>);
    }
    ...
    getDevServerWebpackConfig(builderConfig: BuilderConfiguration<DevServerBuilderOptions>): Observable<webpack.Configuration> {
        const options = builderConfig.options;
        ...
        return checkPort(options.port, options.host).pipe(
            ...
            tap((opts) => this.browserOptions = opts),
            concatMap(() => normalizeFileReplacements(this.browserOptions.fileReplacements, this.host, root)),
            tap(fileReplacements => this.browserOptions.fileReplacements = fileReplacements),
            concatMap(() => normalizeAssetPatterns(
                this.browserOptions.assets, this.host, root, projectRoot, builderConfig.sourceRoot)),
            // Replace the assets in options with the normalized version.
            tap((assetPatternObjects => this.browserOptions.assets = assetPatternObjects)),
            map(() => {
                const webpackConfig = this.buildWebpackConfig(
                    root, projectRoot, this.host, this.browserOptions as NormalizedBrowserBuilderSchema);

                let webpackDevServerConfig: WebpackDevServer.Configuration;
                try {
                    webpackDevServerConfig = this._buildServerConfig(
                        root, projectRoot, options, this.browserOptions);
                } catch (err) {
                ...
                if (options.liveReload) {
                    this._addLiveReload(options, this.browserOptions, webpackConfig, clientAddress);
                } else if (options.hmr) {
                ...
                if (this.browserOptions.optimization) {
            ...
            `);
                ...
                this.opnAddress = serverAddress + webpackDevServerConfig.publicPath;
    ...
    buildWebpackConfig(
        ...
        webpackConfig.target = 'electron-render';
        return webpackConfig;
    }
